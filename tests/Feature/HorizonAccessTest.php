...
it('configures horizon for a serial external-query supervisor', function () {
    expect(config('queue.connections.redis.driver'))->toBe('redis')
        ...
        ->toBeGreaterThan(config('horizon.defaults.supervisor-external-query.timeout'))
        ...
        ->and(config('horizon.defaults.supervisor-external-query.timeout'))->toBe(120)
        ->and(config('horizon.defaults.supervisor-default.timeout'))->toBe(90)
