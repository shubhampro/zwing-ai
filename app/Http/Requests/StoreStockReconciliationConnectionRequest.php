...
class StoreStockReconciliationConnectionRequest extends FormRequest
{
    public function authorize(): bool
    {
        return $this->user()?->can(Permissions::StockReconManage) ?? false;
    }
        ...
        $validator->after(function (Validator $validator): void {
            if (! $this->boolean('include_zwing') && ! $this->boolean('include_erp')) {
                $validator->errors()->add(
                    'include_zwing',
                    __('Select at least one stock side to pull (Zwing and/or ERP).'),
                );
            }
