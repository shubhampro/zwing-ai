...
type Organization = { id: number; name: string; ba_code: string; vendor_id: number };
...
function ConnectionForm({
    ...
    function submit(e: React.FormEvent) {
        ...
        if (isEdit && connection) {
            form.put(updateConnection.url({ organization: organizationId, organizationThirdPartyApi: connection.id }), {
                onSuccess: () => onCancel?.(),
            });
            return;
        }
        ...
        <form onSubmit={submit} className="mt-3 flex flex-col gap-3 border-t pt-3">
            ...
            <div className="space-y-1">
                <Label className="text-xs">
                    Token ({api.auth_header_name}){isEdit ? ' — leave blank to keep' : ''}
                </Label>
                <Input
                    type="password"
                ...
                />
                <InputError message={form.errors.auth_token || form.errors.third_party_api_id} />
            </div>
            ...
            <div className="flex flex-col gap-6 p-4 md:p-6">
                <div className="flex items-start justify-between gap-4">
                    <div>
                        ...
                        <p className="text-sm text-muted-foreground">
                            BA {organization.ba_code} · Vendor {organization.vendor_id}
                        </p>
                    </div>
                ...
                <section className="flex flex-col gap-4">
                    <div>
                        ...
                        <p className="text-sm text-muted-foreground">
                            Set base URL + token per app template for this organization.
                        </p>
                    </div>

                    {apiApps.length === 0 && (
                        <p className="rounded-md border border-dashed px-4 py-8 text-center text-sm text-muted-foreground">
                            No API templates yet. Add templates under Third party APIs first.
                        </p>
                    )}
                            ...
                            >
                                <div className="flex items-start justify-between gap-2">
                                    <div>
                                        ...
                                        <p className="mt-1 text-xs text-muted-foreground">
                                            {api.param_count} params · header {api.auth_header_name}
                                        </p>
                                    </div>
                                    <Badge variant={api.connection?.is_active ? 'default' : 'secondary'}>
                                        {api.connection ? (api.connection.is_active ? 'Connected' : 'Inactive') : 'Not set'}
                                    </Badge>
                                </div>
                                ...
                                {api.connection && editingApiId !== api.id ? (
                                    <div className="mt-3 border-t pt-3">
                                        ...
                                        <p className="font-mono text-sm">{api.connection.base_url}</p>
                                        <p className="mt-2 font-mono text-xs text-muted-foreground">
                                            Full: {api.connection.base_url}
                                            {api.path}
                                        </p>
                                        <div className="mt-3 flex gap-2">
                                            <Button size="sm" variant="outline" onClick={() => setEditingApiId(api.id)}>
                                                Edit
                                            </Button>
                                            <Button
                                                size="sm"
                                                ...
                                                onClick={() =>
                                                    deleteConnection(
                                                        destroyConnection.url({
                                                            ...
                                                            organizationThirdPartyApi: api.connection!.id,
                                                        }),
                                                    )
                                                }
                                            >
                                    ...
                                    <ConnectionForm
                                        ...
                                        api={api}
                                        connection={editingApiId === api.id ? api.connection : null}
                                        onCancel={editingApiId === api.id ? () => setEditingApiId(null) : undefined}
                                    />
                                )}
                            </div>
                        ))}
